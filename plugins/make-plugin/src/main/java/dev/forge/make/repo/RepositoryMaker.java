...
public class RepositoryMaker {
    ...
    public static void make(String pkg, String name, String template, boolean override) {
        ...
        base = base.resolve(config.getString("make.packages.repository", "repositories"));

        String suffix = config.getBoolean("make.addSuffix", true) ? "Repository.java" : ".java";

        base = base.resolve(name + suffix);
        template = template != null && !template.isBlank() ? template
                : config.getString("make.templates.repository", "templates/repository.mustache");
        ...
        new FileMaker().override(override).generate(base, template, Map.of(
                "package", pkg + "." + config.getString("make.packages.repository", "repositories"),
                "className", name,
